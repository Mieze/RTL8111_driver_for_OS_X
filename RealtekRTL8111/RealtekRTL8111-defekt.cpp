...
UInt32 RTL8111::outputPacket(mbuf_t m, void *param)
{
    ...
    if (!(isEnabled && linkUp)) {
        DebugLog("Interface or link down. Dropping packet.\n");
        goto error1;
    }
...
IOReturn RTL8111::setMulticastList(IOEthernetAddress *addrs, UInt32 count)
{
    ...
    DebugLog("Old multicastFilter=0x%llx\n", multicastFilter);
    ...
    DebugLog("New multicastFilter=0x%llx\n", multicastFilter);
    DebugLog("setMulticastList() <===\n");
...
bool RTL8111::initChip(struct rtl8168_private *tp)
{
    ...
    tp->intr_mask = LinkChg | RxDescUnavail | TxErr | TxOK | RxErr | RxOK | PCSTimeout;
    /*
...
void RTL8111::timerAction(IOTimerEventSource *timer)
{
    ...
    if (linkUp) {
        ...
        if (ReadReg8(ChipCmd) & CmdRxEnb) {
            ...
            /* Start the chip's timer to interrupt when the dump has been completed. */
            //WriteReg32(TCTR, 0);
            //WriteReg32(TimeIntr, 12500);
        }
...
void RTL8111::updateStatitics()
{
    ...
    if (!(ReadReg32(CounterAddrLow) & CounterDump)) {
        netStats->inputPackets = (UInt32)OSSwapLittleToHostInt64(statData->rx_packets);
        netStats->inputErrors = OSSwapLittleToHostInt32(statData->rx_errors);
        netStats->outputPackets = (UInt32)OSSwapLittleToHostInt64(statData->tx_packets);
        netStats->outputErrors = OSSwapLittleToHostInt32(statData->tx_errors);
        
        sgColl = OSSwapLittleToHostInt32(statData->tx_one_collision);
        mlColl = OSSwapLittleToHostInt32(statData->tx_multi_collision);
        ...
        etherStats->dot3StatsEntry.alignmentErrors = OSSwapLittleToHostInt16(statData->align_errors);
        etherStats->dot3StatsEntry.missedFrames = OSSwapLittleToHostInt16(statData->rx_missed);
        etherStats->dot3TxExtraEntry.underruns = OSSwapLittleToHostInt16(statData->tx_underun);
    }
}
# if DISABLED_CODE
void RTL8111::timerAction(IOTimerEventSource *timer)
{
    ...
    if (!linkUp) {
        DebugLog("Can't update statistics while link is down.\n");
        goto done;
    }
    if (updateStats) {
        updateStatitics();
        DebugLog("Statistics haven't been updated during the last second.\n");
    }
...
*/
        ...
        if (ReadReg8(ChipCmd) & CmdRxEnb) {
            ...
            /* Start the onchip timer to interrupt after 10ms. */
            WriteReg32(TCTR, 0);
            WriteReg32(TimeIntr, txTimeoutValue);
            updateStats = true;
        }
        /* Schedule IOTimerEventSource and reset deadlockWarn. */
        deadlockWarn = 0;
    }
...
void RTL8111::updateStatitics()
{
    ...
    /* Stop the onchip timer. */
    WriteReg32(TCTR, 0);
    WriteReg32(TimeIntr, 0);
    ...
    if (!(ReadReg32(CounterAddrLow) & CounterDump)) {
        ...
        DebugLog("Tally counter dump didn't complete within 10ms.\n");
    }
}
#endif  /* DISABLED_CODE */
...
void RTL8111::checkLinkStatus()
{
	...
	if (currLinkState & LinkStatus) {
        ...
        deadlockWarn = 0;
        txQueue->start();
        IOLog("Ethernet [RealtekRTL8111]: Link up on %s%u, %s, %s, %s\n", name, unitNumber, speedName, duplexName, flowName);
	} else {
        setLinkStatus(kIONetworkLinkValid);
        txQueue->stop();
        txQueue->flush();
        ...
        txClear();
        IOLockUnlock(txLock);
        IOLog("Ethernet [RealtekRTL8111]: Link down on %s%u\n", name, unitNumber);
...
void RTL8111::fillDescriptorAddr()
{
    UInt32 *descAddr;
    
    descAddr = (UInt32 *)&txPhyAddr;
    WriteReg32(TxDescStartAddrLow, *descAddr++);
    WriteReg32(TxDescStartAddrHigh, *descAddr);
    descAddr = (UInt32 *)&rxPhyAddr;
    WriteReg32(RxDescAddrLow, *descAddr++);
    WriteReg32(RxDescAddrHigh, *descAddr);
}
